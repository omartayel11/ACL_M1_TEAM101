#!/usr/bin/env python3
"""
Comprehensive Quantitative Test for Intent Classifier
Tests the hybrid intent classifier with diverse real-world queries including typos and variations
...
class IntentClassifierTester:
    """
    Comprehensive tester for intent classification with accuracy metrics
    Supports checkpoint/resume and live progress display
    """
    
    CHECKPOINT_FILE = "intent_classifier_checkpoint.json"
    ...
    def run_tests(self, verbose=False, resume=True):
        ...
        if start_index == 0:
            print("=" * 80)
            print("INTENT CLASSIFIER QUANTITATIVE TEST")
            print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Quantitative test for intent classifier")
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
    ...
    parser.add_argument(
        "-o", "--output",
        type=str,
        default="intent_classifier_results.txt",
        help="Output file for results (default: intent_classifier_results.txt)"
    )
    parser.add_argument(
        "--no-resume",
        action="store_true",
    ...
    print("\n" + "=" * 80)
    print("✅ Intent classifier test completed!")
