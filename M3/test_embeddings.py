...
print(f"Review index loaded: {searcher.review_index is not None}")
print(f"Hotel mapping size: {len(searcher.hotel_mapping) if searcher.hotel_mapping else 0}")
print(f"Review mapping size: {len(searcher.review_mapping) if searcher.review_mapping else 0}\n")

# Try search with different thresholds
print("=" * 60)
for threshold in [0.9, 0.7, 0.5, 0.3, 0.0]:
    print(f"\nSearching with threshold={threshold}:")
    results = searcher.search(embedding, limit=5, threshold=threshold, search_type="hotels")
    print(f"  Found {len(results)} results")
    
    if results:
        for i, r in enumerate(results[:3], 1):
            print(f"    {i}. {r.get('name', 'N/A')} - similarity: {r.get('similarity_score', 0):.4f}")
